...
const Index = () => {
  ...
  const addPost = (title, body) => {
    const newPost = { id: posts.length + 1, title, body, date: new Date(), author: user.username, reactions: {} };
  ...
  const toggleReaction = (postId, emoji) => {
    const updatedPosts = posts.map(post => {
      if (post.id === postId) {
        ...
        post.reactions[emoji] = currentCount ? 0 : 1;
      }
      return post;
    });
  ...
  return (
    <Container maxW="container.md" p={4}>
      ...
      {user && (
        <Box mb={4}>
          <Button leftIcon={<FaPlus />} colorScheme="teal" onClick={() => addPost("New Post", "This is a new post.")}>
            New Post
          </Button>
        </Box>
      )}
